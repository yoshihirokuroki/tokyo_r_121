# Model-Informed Drug Development と R
...
発表資料は [github.io](https://yoshihirokuroki.github.io/tokyo_r_121/) にて公開しています。
...
> **macOS でのグラフィックデバイスについて** 標準の `svg` デバイスは Cairo/X11 に依存し、XQuartz が無いとビルドに失敗します。本プロジェクトは `dev: "ragg_png"` を指定して `ragg` を使うため、XQuartz のインストールは不要です。
